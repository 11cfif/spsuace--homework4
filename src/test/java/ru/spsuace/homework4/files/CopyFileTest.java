...
import ru.spsuace.homework4.streams.CopyFile;
...
//        // --------------------------------------- create source for copyDirHard ---------------------------------------
    ...
    public void copyOnlyFile() throws IOException {
        ...
        CopyFile.copyFile(file.toString(), dest.toString());
    ...
    public void copyOnlyDir() throws IOException {
        ...
        CopyFile.copyDir(source.toString(), dest.toString());
    ...
    private void checkCopy(Path source, Path dest, boolean isDirectory) throws IOException {
        ...
        for (int i = 0; i < sourcePaths.size(); i++) {
            ...
            if (Files.isRegularFile(sourcePaths.get(i))) {
                checkFileCopy(sourcePaths.get(i), destPaths.get(i));
            }
        }
    }

    @Test
    public void copyBigFiles() {
        try {
            Path file = Paths.get("src", "test", "resources", "directories", "copy", "copyBigFiles", "file.txt");
            Files.createDirectories(file.getParent());
            createFile(file);
            Path fileRes = Paths.get("src", "test", "resources", "directories", "copy1", "copyBigFiles", "file.txt");
            CopyFile.copyFile(file.toString(), fileRes.toString());
            checkFileCopy(file, fileRes);
        } catch (IOException e) {
            fail(e.getMessage());
        }
    }

    private void checkFileCopy(Path source, Path dest) {
        try {
            assertEquals(Files.size(source), Files.size(dest));
            List<String> sourceLines = Files.readAllLines(source);
            List<String> destLines = Files.readAllLines(dest);
            assertEquals(sourceLines, destLines);
        } catch (IOException e) {
            fail(e.getMessage());
        }
    }
